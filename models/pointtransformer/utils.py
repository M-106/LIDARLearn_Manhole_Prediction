    ...
    def forward(self, pxo1, pxo2=None, y=None):
        if pxo2 is None:
            ...
            for i in range(o.shape[0]):
                ...
                x_b = x[s_i:e_i, :]
                if self.num_shape_class is not None:
                    # Part seg: concat features with global-pool + shape-class one-hot.
                    y_b = y[i].unsqueeze(-1).unsqueeze(-1).long()
                    y_onehot = torch.zeros(1, self.num_shape_class, device=x.device)
                    y_onehot.scatter_(1, y_b, 1)  # (1, l)
                    x_b = torch.cat((x_b,
                                     self.linear2(x_b.sum(0, True) / cnt).repeat(cnt, 1),
                                     self.linear3(y_onehot).repeat(cnt, 1)), dim=1)
                else:
                    # Semantic seg: concat features with global-pool only (no label).
                    x_b = torch.cat((x_b,
                                     self.linear2(x_b.sum(0, True) / cnt).repeat(cnt, 1)), dim=1)
                x_tmp.append(x_b)
            x = torch.cat(x_tmp, 0)
            x = self.linear1(x)
        else:
