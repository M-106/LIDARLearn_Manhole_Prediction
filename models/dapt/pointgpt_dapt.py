...
from .gpt_dapt import GPT_extractor, GPT_generator
from .z_order_gpt_dapt import *
...
class PointGPT_DAPT(BasePointCloudModel):
    def __init__(self, config, **kwargs):
        num_classes = config.cls_dim
        super(PointGPT_DAPT, self).__init__(config, num_classes)
    ...
    def load_model_from_ckpt(self, bert_ckpt_path):
        if bert_ckpt_path is not None:
            ...
            print_log(f'[PointGPT_DAPT] Loaded {len(matched_keys)}/{len(base_ckpt)} parameter groups ({used_percentage:.2f}% of checkpoint parameters)', logger='Transformer')
            ...
            print_log(
                f'[PointGPT_DAPT] Successful Loading the ckpt from {bert_ckpt_path}', logger='Transformer')
        else:
    ...
    def forward(self, pts):
        ...
        ret, concat_f = self.blocks(x, pos, classify=True)
