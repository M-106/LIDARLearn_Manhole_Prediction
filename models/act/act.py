"""
ACT: Autoencoders as Cross-Modal Teachers: Can Pretrained 2D Image Transformers Help 3D Representation Learning?

Paper: ICLR 2023
Authors: Runpei Dong, Zekun Qi, Linfeng Zhang, Junbo Zhang, Jianjian Sun, Zheng Ge, Li Yi, Kaisheng Ma
Source: Implementation adapted from: https://github.com/RunpeiDong/ACT
License: MIT
"""
...
from .act_3d_dvae import Group, Encoder, ACTPromptedDiscreteVAEwithVIT
from ..build import MODELS
from ..base_model import BasePointCloudModel, print_model_info
from utils.checkpoint import get_missing_parameters_message, get_unexpected_parameters_message
from utils.logger import print_log

from ..ssl_blocks import Mlp, Attention, Block, TransformerEncoder
...
class ACT(BasePointCloudModel):
    def __init__(self, config, **kwargs):
        num_classes = config.cls_dim
        super(ACT, self).__init__(config, num_classes)
        ...
        if config.transfer_type != 'full':
            for name, param in self.named_parameters():
                if config.transfer_type == 'mlp-3' or config.transfer_type == 'linear':
                    ...
                    if not 'side' in name and not 'cls' in name:
                        param.requires_grad = False
                elif config.transfer_type == 'bit-fit':
                    if not 'bias' in name and not 'cls' in name:
                        param.requires_grad = False
    ...
    def load_model_from_ckpt(self, bert_ckpt_path, custom_loading=False):
        if bert_ckpt_path is not None:
            ...
            print_log(f'[ACT] Loaded {len(matched_keys)}/{len(base_ckpt)} parameter groups ({used_percentage:.2f}% of checkpoint parameters)', logger='Transformer')
            ...
            print_log(f'[ACT] Successful Loading the ckpt from {bert_ckpt_path}', logger='Transformer')
        else:
...
@MODELS.register_module()
class ACT_PointDistillation_Pretrain(nn.Module):
