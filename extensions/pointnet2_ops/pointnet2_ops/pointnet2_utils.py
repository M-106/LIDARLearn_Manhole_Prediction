...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample, xyz, new_xyz):
        # type: (Any, int, torch.Tensor, torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]
        r"""
        ...
        new_xyz : torch.Tensor
            (B, npoint, 3) query points

        Returns
        -------
        dist : torch.Tensor
            (B, npoint, nsample) l2 distance to the k nearest neighbors
        idx : torch.Tensor
            (B, npoint, nsample) indices of k nearest neighbors
        """
        dist2, idx = _ext.knn_query(xyz, new_xyz, nsample)
        dist = torch.sqrt(dist2)
...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, xyz, new_xyz, features=None):
        ...
        if self.radius is not None:
            ...
            _,idx = knn_query(self.nsample, xyz, new_xyz)
