"""
dela_cutils - CUDA utilities for DeLA model

This module provides pre-compiled CUDA operations for:
- Grid subsampling
- KD-Tree operations
- KNN edge maxpooling
"""
...
def grid_subsampling(xyz: torch.Tensor, grid_size: float, hash_size: float=1.) -> torch.Tensor:
    ...
    indices = _C.grid_subsampling(xyz, grid_size, table, storage)
    return indices
...
def grid_subsampling_test(xyz: torch.Tensor, grid_size: float, hash_size: float=1., pick=0) -> torch.Tensor:
    ...
    indices = _C.grid_subsampling_test(xyz, grid_size, table, storage, pick)
    return indices
...
class KDTree():
    ...
    def __init__(self, xyz: torch.Tensor, max_leaf_size=20):
        ...
        self.xyz = xyz
        self.n = xyz.shape[0]
        self.tree, self.pca = _C.kdtree_build(xyz, max_leaf_size)
    
    def __del__(self):
        _C.kdtree_free(self.tree, self.pca)
    ...
    def knn(self, query: torch.Tensor, k=1, ordered=False):
        ...
        _C.kdtree_knn(self.tree, query, indices, dists, ordered)
        if self.n < k:
...
class KEMP(Function):
    ...
    def forward(ctx, feature: torch.Tensor, knn: torch.Tensor, training: bool=True) -> torch.Tensor:
        ...
        if training or feature.requires_grad:
            ...
            if feature.dtype == torch.half:
                _C.half_aligned_knn_edge_maxpooling_forward(output, indices, feature, knn)  
            else:
                _C.aligned_knn_edge_maxpooling_forward(output, indices, feature, knn)
            ctx.save_for_backward(indices)
        else:
            if feature.dtype == torch.half:
                _C.half_aligned_knn_edge_maxpooling_infer(output, feature, knn)
            else:
                _C.aligned_knn_edge_maxpooling_infer(output, feature, knn)
        return output
    ...
    def backward(ctx, grad: torch.Tensor):
        ...
        if grad.dtype == torch.half:
            _C.half_knn_edge_maxpooling_backward(output, indices, grad)  
        else: 
            _C.knn_edge_maxpooling_backward(output, indices, grad)
