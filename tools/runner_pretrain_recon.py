...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    extra_test_dataloader = None
    if config.validate != "none":
        (_, extra_train_dataloader), (_, extra_test_dataloader) = (
    ...
    # CLI --augmentation overrides the yaml's pretrain_augmentation (same
    # behavior as runner_pretrain / runner_finetune).
    ...
    for epoch in epoch_pbar:
        ...
        for idx, (taxonomy_ids, model_ids, pc, img, text, _) in enumerate(batch_pbar):
            num_iter += 1
        ...
        if config.validate != "none" and epoch % args.val_freq == 0:
            metrics = validate_svm(
                base_model, extra_train_dataloader, extra_test_dataloader,
                epoch, args, config,
                npoints=config.dataset.train.others.npoints,
                svm_kwargs={'C': 0.075},
                return_percent=False,
                val_writer=val_writer, writer_tag='Metric/ACC',
                logger=logger,
            )
