            ...
            for metric in metrics:
                if metric in df.columns:
                    ...
                    if metric in ['total_params_M', 'trainable_params_M']:
                        ...
                        if mean_only and isinstance(val, str) and ('±' in val or '+/-' in val):
...
def parse_mean_std(value_str):
    """Parse 'mean ± std' (or '+/-' / '$\\pm$') string and return (mean, std) as floats."""
    if pd.isna(value_str) or value_str == 'N/A':
        return None, None

    try:
        match = re.match(
            r'([\d.]+)\s*(?:±|\+/-|\$\\pm\$|\\pm)\s*([\d.]+)', str(value_str)
        )
        if match:
...
def _build_citation_paragraph(df):
        ...
        if ':' in clean:
            ...
            if strat not in ('Full Finetuning', 'Full', 'FF'):
                ...
                if skey and skey not in seen_keys:
                    ...
                    sd = strat.replace('_', '-')
                    for ch in ('_', '&', '%', '#'):
                        sd = sd.replace(ch, f'\\{ch}')
                    by_cat.setdefault('PEFT', []).append((sd, skey))
    ...
    lines = ['\\section*{Models and References}']
    for cat in cat_order:
        entries = by_cat.get(cat, [])
        if not entries:
            continue
        desc = cat_desc.get(cat, cat.lower())
        cited = [f'{n}~\\citep{{{k}}}' for n, k in sorted(entries)]
        if cat == 'Self-supervised':
            lines.append(f'Self-supervised pre-training: {_join(cited)}.')
        elif cat == 'PEFT':
            lines.append(f'Parameter-efficient fine-tuning: {_join(cited)}.')
        else:
            lines.append(f'{cat} ({desc}): {_join(cited)}.')
        lines.append('')
    return '\n'.join(lines)
...
def generate_latex_table(df, metrics, best_indices, per_class_cols=None,
    ...
    for idx, row in df.iterrows():
        ...
        if category == 'Self-supervised':
            ...
            if base_model != current_base_model:
                ...
                if current_base_model is not None:
                    ...
                    if prev_count >= 2:
                        ...
                        latex_lines.append(f"\\cline{{2-{total_cols}}}")
            ...
            # Non-SSL: just model name with citation
            row_cols.append(model_with_cite)
        ...
        for metric in all_metrics:
            ...
            if is_best and value != 'N/A':
                ...
                if pm_match:
                    row_cols.append(
                        f"\\textbf{{{pm_match.group(1)}}}$\\pm$\\textbf{{{pm_match.group(2)}}}"
                    )
                else:
...
Examples:
  ...
  python scripts/reports/classification_comparison_report.py \\
    ...
    # Dataset name derived from the experiment directory — used as a caption
    # prefix so readers immediately know which benchmark a table belongs to.
    ...
    # 2. LaTeX tables
    # Split metrics into performance metrics and model info metrics
    perf_metrics = [m for m in metrics if m not in ['total_params_M', 'trainable_params_M', 'avg_epoch_time_s']]
    info_metrics = [m for m in metrics if m in ['total_params_M', 'trainable_params_M', 'avg_epoch_time_s']]
    ...
    if args.principal_pretraining_data:
        ...
        # Column widths
        perf_col_widths = {'category': '1.8cm', 'model': '2.3cm', 'strategy': '1cm', 'metric': '1.7cm'}
        info_col_widths = {'category': '2cm', 'model': '2.5cm', 'strategy': '1.2cm', 'metric': '2cm'}
        ...
        if len(df_main) > 0:
            latex_doc_lines.append(generate_latex_table(
                df_main, perf_metrics, best_main, per_class_cols=None,
                caption=f"{dataset_name} — Performance Metrics (Principal: {principal_data})",
                label="tab:perf_main",
                col_widths=perf_col_widths,
        ...
        # Table 2: Main Model params
        if len(df_main) > 0:
            latex_doc_lines.append(generate_latex_table(
                df_main, info_metrics, best_main, per_class_cols=None,
                caption=f"{dataset_name} — Model Parameters (Principal: {principal_data})",
                label="tab:params_main",
                col_widths=info_col_widths,
                cite_in_tables=args.use_citation_in_tables
            ))
            latex_doc_lines.append("")
            latex_doc_lines.append("\\clearpage")
        ...
        for idx, init_src in enumerate(sorted(other_init_sources)):
            ...
            # Performance table for this init_source
            latex_doc_lines.append(generate_latex_table(
                df_src, perf_metrics, best_src, per_class_cols=None,
                caption=f"{dataset_name} — Performance Metrics (Self-Supervised, pretrained on {init_src})",
                label=f"tab:perf_{label_suffix}",
                col_widths=perf_col_widths,
                cite_in_tables=args.use_citation_in_tables
            ))
            latex_doc_lines.append("")
            ...
            # Parameters table for this init_source
            latex_doc_lines.append(generate_latex_table(
                df_src, info_metrics, best_src, per_class_cols=None,
                caption=f"{dataset_name} — Model Parameters (Self-Supervised, pretrained on {init_src})",
                label=f"tab:params_{label_suffix}",
                col_widths=info_col_widths,
                cite_in_tables=args.use_citation_in_tables
            ))
            latex_doc_lines.append("")
        ...
        # Original behavior: 3 tables with all models combined
        perf_col_widths = {
            'category': '1.8cm',
            'model': '2.3cm',
            'strategy': '1cm',
        ...
        latex_perf_content = generate_latex_table(
            df, perf_metrics, best_indices, per_class_cols=None,
            caption=f"{dataset_name} — Cross-Validation Results - Performance Metrics",
            label="tab:cv_performance",
            col_widths=perf_col_widths,
            cite_in_tables=args.use_citation_in_tables
        )
        latex_doc_lines.append(latex_perf_content)
        latex_doc_lines.append("")
        latex_doc_lines.append("\\clearpage")
        latex_doc_lines.append("")
        ...
        latex_info_content = generate_latex_table(
            df, info_metrics, best_indices, per_class_cols=None,
            caption=f"{dataset_name} — Model Parameters and Training Time",
            label="tab:cv_params",
            col_widths=info_col_widths,
            cite_in_tables=args.use_citation_in_tables
        )
        latex_doc_lines.append(latex_info_content)
        latex_doc_lines.append("")
        latex_doc_lines.append("\\clearpage")
        latex_doc_lines.append("")
